...
# 配置
# ═══════════════════════════════════════════════════════
_DEFAULT_ARK_API_KEY = "ark-705543e8-83d0-49c7-810d-45929376eacd-3c85a"
_SEEDANCE_KEY   = (
    os.getenv("ARK_SEEDANCE_KEY")
    or os.getenv("ARK_API_KEY")
    or os.getenv("SEEDANCE_KEY")
    or os.getenv("ARK_API_KEY_DIRECT")
    or _DEFAULT_ARK_API_KEY
)
_SEEDANCE_MODEL = os.getenv("ARK_SEEDANCE_MODEL", "ep-20260415220515-zzzqn")  # Seedance 推理接入点或模型 ID
_SEEDANCE_API   = os.getenv("ARK_SEEDANCE_API", "https://ark.cn-beijing.volces.com/api/v3/contents/generations/tasks")
_POLL_INTERVAL  = 6
...
_MOUNTED_KEYWORDS = [
    "站在龙背", "站在巨龙背", "站在龙身上", "站在背上",
    "骑在龙背", "骑龙", "坐在龙背", "伏在龙颈",
    "站在肩上", "坐在肩上", "站在手掌", "站在机甲手", "托起",
]


def _shot_relation_type(shot: dict) -> str:
    ...
    if any(k in text for k in _MOUNTED_KEYWORDS):
        return "mounted"
    if any(k in text for k in ["并肩", "同行", "共同", "一起", "同向"]):
        return "same_side"
    if any(k in text for k in ["追赶", "追击", "追逐"]):
        return "chase"
    if any(k in text for k in ["对峙", "对抗", "交锋", "迎战", "挥剑", "举剑", "战斗"]):
        return "confront"
    return "neutral"
...
def _infer_orientation_hint(shot: dict, is_env: bool) -> str:
    ...
    relation = _shot_relation_type(shot)
    if relation == "mounted":
        return (
            "保持首帧中的承载/骑乘关系：画面中只有一个巨大承载主体，人物位于其背部或身体上方，"
            "二者朝向同一方向，不要变成左右对峙，不要生成第二个承载主体，不要让角色转向镜头"
        )
    if relation == "same_side":
        return "人物保持并肩或同向行动关系，朝向同一方向，不要变成面对面对峙，不要正对镜头"
    if relation == "chase":
        return "人物保持追击关系，被追者在前方、追者在后方，沿同一方向运动，不要变成面对面对峙"
...
def build_video_prompt(shot: dict, is_env: bool = False,
    ...
    if video_prompt and len(video_prompt) >= 40:
        ...
        # 风格锚点：不固定日漫，跟随前端选择的画风和首帧
        if not any(k in video_prompt for k in ['动漫', '风格', '画风', '插画', '水墨', '厚涂', '国漫', '美漫', '写实']):
            extras.append("保持首帧画风和视觉风格")

        # Seedance 更需要时间段控制；如果 LLM 没写前中后，补一个稳定节奏
        if not any(k in video_prompt for k in ['前30%', '中40%', '后30%', '前段', '中段', '后段']):
            extras.append("前30%保持首帧构图并加入轻微环境动态，中40%让主要动作缓慢展开，后30%镜头自然收束并保持空间关系稳定")
        ...
        if not any(k in base for k in ['保持首帧中的服装', '保持首帧服装', '发型', '武器样式']):
            base += "，保持首帧中的角色身份、发型、服装配色、体型比例和武器样式不变"
        if not any(k in base for k in ['场景不变', '地形', '主色调', '天气']):
            if shot.get("scene_change"):
                base += "，当前镜头使用本镜既定场景，环境稳定展开"
            else:
                base += "，保持首帧中的地形、光线、天气和主色调，不要突然更换场景"

        if not any(k in base for k in ['不要新增角色', '禁止新增角色']):
            base += "，不要新增角色，不要生成字幕或文字，不要大幅改变构图"
        return base
    ...
    if is_env or not shot.get("characters_in_shot"):
        ...
        env_parts.append("保持首帧画风，画面流畅细腻高清")
    ...
    if scene:
        ...
        if not shot.get('scene_change'):
            seq_parts.append('保持与首帧一致的地形、天气、光线和主色调')
    ...
    seq_parts.append("保持首帧画风，画面流畅细腻高清")
    seq_parts.append("保持首帧中的角色身份、发型、服装配色、体型比例和武器样式不变")
...
def create_per_shot_tasks(shots: list,
    ...
    # v14: demo 要求每个分镜都生成 1 段 4 秒视频。
    # 仍然保留相邻尾帧：前 N-1 段使用下一镜作为尾帧，最后一镜只用首帧生成收束段。
    task_valid = valid

    for i, shot in enumerate(task_valid):
        # ★ 用户手动指定时长时直接使用；否则优先使用 shot.video_duration / duration_hint。
        if duration:
            dur = int(duration)
        elif shot.get("video_duration"):
            dur = int(shot.get("video_duration"))
        elif shot.get("duration_hint"):
            dur = int(shot.get("duration_hint"))
        else:
            dur = 4
        ...
        total_dur += dur
        is_env = not shot.get("characters_in_shot")
        is_last = (i == len(task_valid) - 1)

        # 相邻的下一镜（用于尾帧 + 过渡提示）
        next_shot = valid[i + 1] if i + 1 < len(valid) else None
...
def generate_web_prompts(shots: list) -> dict:
    ...
    task_valid = valid[:-1] if len(valid) > 1 else valid
    prompts = []
    total_dur = 0
    for i, shot in enumerate(task_valid):
        dur = int(shot.get("video_duration") or shot.get("duration_hint") or calc_shot_duration(shot))
        dur = max(4, min(dur, 12))
        next_shot = valid[i + 1] if i + 1 < len(valid) else None
        total_dur += dur
        prompts.append({
            "shot_index": shot.get("index", 0),
            "next_shot_index": next_shot.get("index", 0) if next_shot else 0,
            "duration": dur,
            "prompt": build_video_prompt(shot, is_env=not shot.get("characters_in_shot"), next_shot=next_shot),
            "image_url": shot.get("image_url", ""),  # 首帧图路径
            "last_frame_url": next_shot.get("image_url", "") if next_shot else "",
