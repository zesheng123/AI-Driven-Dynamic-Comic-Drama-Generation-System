...
★ v11 核心修复 ★
  ...
  2. 【引擎切换】优先用豆包 Seedream 生成全景(质量更高,色彩更好)
     - NAI 仅作降级
  3. 【智能尺寸】
     - 室内: 1:1 (裁切视角饱满)
     - 户外/全景: 16:9 (更开阔)
...
def generate_equirect_panorama(scene_spec: str, style: str = "日漫",
    ...
    if engine == "doubao":
        ...
        if r.get("success"):
            return r
        print(f"[panorama] 豆包失败, 降级到 NAI: {r.get('message','')}")

    # ── 4. 降级 NAI ──
    return _generate_panorama_nai(scene_text_en, style, scene_info=scene_info)
...
def _generate_panorama_nai(scene_text_en: str, style: str,
                            scene_info: dict = None) -> dict:
    """v11: NAI 生成全景图 (降级方案)"""
    from services.image_service import _nai_generate, STYLE_PREFIX, _QUALITY, _BASE_NEG

    prefix = STYLE_PREFIX.get(style) or "anime illustration"

    if not scene_text_en or len(scene_text_en) < 10:
        scene_text_en = "detailed scenery, atmospheric lighting"

    prompt = (
        f"{prefix}, "
        f"{scene_text_en}, "
        f"wide establishing shot, detailed scenery, "
        f"no humans, scenery only, "
        f"detailed background, volumetric lighting, atmospheric, {_QUALITY}"
    )
    neg = _BASE_NEG + ", 1girl, 1boy, people, person, human, face, body, figure, crowd, character"

    si = scene_info or {'width': 1024, 'height': 1024}
    w, h = si['width'], si['height']

    print(f"[panorama-nai] prompt: {prompt[:200]}...")
    result = _nai_generate(
        prompt=prompt, negative_prompt=neg,
        width=w, height=h, seed=42,
    )
    if not result.get("success"):
        return {"success": False, "message": result.get("message", "NAI 场景图失败")}

    url = result.get("url", "")
    local_name = f"pano_{uuid.uuid4().hex[:8]}.png"
    local_path = os.path.join(PANO_DIR, local_name)
    base_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    src = os.path.join(base_dir, url.lstrip("/"))
    if os.path.exists(src):
        import shutil
        shutil.copy2(src, local_path)

    print(f"[panorama-nai] ✓ {local_path}")
    return {
        "success": True,
        "local_path": local_path,
        "url": f"/static/panoramas/{local_name}",
