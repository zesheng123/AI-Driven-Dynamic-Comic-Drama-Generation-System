...
def seedance_create_per_shot_tasks():
    ...
    # demo 固定每镜 4 秒；前端传入其它值时尊重前端。
    duration_raw = data.get('duration', 4)
    duration = int(duration_raw) if duration_raw else 4
    # 兼容旧前端可能传 combat_expand，但当前 service 不再需要该参数。
    return jsonify(svc.create_per_shot_tasks(
        shots, resolution=resolution, ratio=ratio, duration=duration
    ))
